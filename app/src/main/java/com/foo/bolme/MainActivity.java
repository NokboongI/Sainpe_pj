...
public class MainActivity extends AppCompatActivity {
    ...
    private void addNewButton(String name) {
        ...
        mainBinding.scrollLayout.addView(newButton);
        buttonNames.add(name);
    }

    //앱 종류 재 시작시에도 버튼이 계속 유지되도록 번튼에 대한 정보 저장 및 복구
        ...
        if (cursor != null && cursor.moveToFirst()) {
            ...
            do {
                if (addressIndex != -1 && bodyIndex != -1) {
                    ...
                    String name = contacts.get(phoneNumber);
                    // 중복 체크 및 새 메시지 추가, 전화번호 또는 연락처의 이름으로 온 것이 있냐 없냐
                    boolean exist = false;
                    ...
                    if (!exist) {
                        if(contacts.containsKey(phoneNumber)){
                            ...
                            Message newMessage = new Message(null,phoneNumber, message, Message.TYPE_RIGHT);
                            messages.add(newMessage);
                        }
